...
func MileStoneSubscriberCBV2(subj, reply string, m *subscribers.MileStone) {
	...
	if m.MileStone == constants.MileStones["500_COIN_MILESTONE"] {
		titleTemplate := "500_milestone_title"
		textTemplate := "500_milestone_text"
		data := i18n.DataModel{
			Name: profile.Name,
		}
		...
		bigPictureTemplateName := "500_milestone_image"
		...
		notification := mongo.CreateNotification(mongo.NotificationModel{
			Receiver:        profile.Id,
			Identifier:      profile.Id.Hex() + "500_coin_milestone",
	...
	if m.MileStone == constants.MileStones["2500_COIN_MILESTONE"] {
		titleTemplate := "2500_milestone_title"
		textTemplate := "2500_milestone_text"
		data := i18n.DataModel{
			Name: profile.Name,
		}
		msgStrTitle := i18n.GetString(profile.Language, titleTemplate, data)
		msgStrText := i18n.GetString(profile.Language, textTemplate, data)
		htmlMsgStr := i18n.GetString(profile.Language, titleTemplate, data)
		bigPictureTemplateName := "2500_milestone_image"
		bigPicture := i18n.GetString(profile.Language, bigPictureTemplateName, data)
		...
		notification := mongo.CreateNotification(mongo.NotificationModel{
			Receiver:        profile.Id,
			Identifier:      profile.Id.Hex() + "2500_coin_milestone",
			Participants:    []bson.ObjectId{profile.Id},
			DisplayTemplate: constants.NotificationTemplate["TRANSACTIONAL"],
			EntityGroupId:   profile.Id.Hex(),
			ActionId:        profile.Id,
			ActionType:      "coin_milestone",
			Purpose:         constants.NotificationPurpose["2500_COIN_MILESTONE"],
			Message:         msgStrTitle,
			MessageMeta:     messageMeta,
			MessageHtml:     htmlMsgStr,
			DeepLink:        deepLink,
